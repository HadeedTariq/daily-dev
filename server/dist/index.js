...
var env = (0, import_envalid.cleanEnv)(process.env, {
  ...
  SERVER_DOMAIN: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("http://localhost:3000") }),
  ...
  DATABASE_URL: (0, import_envalid.url)({ devDefault: (0, import_envalid.testOnly)("http://localhost:3000") }),
  REDIS_URL: (0, import_envalid.url)({ devDefault: (0, import_envalid.testOnly)("http://localhost:5173") }),
  DATABASE_HOST: (0, import_envalid.host)({ devDefault: (0, import_envalid.testOnly)("localhost") }),
  DATABASE_PORT: (0, import_envalid.port)({ devDefault: (0, import_envalid.testOnly)(3e3) }),
  DATABASE_USER: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("user") }),
  DATABASE_PASSWORD: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("password") }),
  PASSWORD_SALT: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("salt") }),
  JWT_SECRET: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("secret") }),
  JWT_REFRESH_TOKEN_SECRET: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("secret") }),
  JWT_ACCESS_TOKEN_SECRET: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("secret") }),
  GITHUB_CLIENT_ID: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("secret") }),
  GITHUB_CLIENT_SECRET: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("secret") }),
  NODE_MAILER_USER: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("user") }),
  SESSION_SECRET: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("user") }),
  NODE_MAILER_PASSWORD: (0, import_envalid.str)({ devDefault: (0, import_envalid.testOnly)("password") })
});
...
function generateOpenAPIDocument() {
  const registry = new import_zod_to_openapi2.OpenAPIRegistry([healthCheckRegistry]);
  const generator = new import_zod_to_openapi2.OpenApiGeneratorV3(registry.definitions);
...
var emailQueue = new import_bullmq.Queue("emailQueue", { connection: redis });
...
var UserController = class {
  ...
  async createUser(req, res, next) {
    ...
    try {
      ...
      const user = (0, import_jsonwebtoken.verify)(decryptedToken, env.JWT_SECRET);
  ...
  async registerUser(req, res, next) {
    ...
    const signedToken = (0, import_jsonwebtoken.sign)(dataStoredInToken, env.JWT_SECRET, { expiresIn });
  ...
  async authenticateByResfreshToken(req, res, next) {
    ...
    const { id } = (0, import_jsonwebtoken.verify)(refToken, env.JWT_REFRESH_TOKEN_SECRET);
    if (!id) {
  ...
  encryptToken = (token) => {
    ...
    const cipher = (0, import_crypto.createCipheriv)(algorithm, key, initVector);
  ...
  decryptToken = (token) => {
    ...
    const algorithm = "aes-256-cbc";
    const initVector = Buffer.from("1234567890abcdef");
    const decipher = (0, import_crypto.createDecipheriv)(algorithm, key, initVector);
    return decipher.update(token, "hex", "utf8") + decipher.final("utf8");
  };
  hashPassword = async (password) => {
    const hashPassword = await (0, import_bcrypt.hash)(password, 16);
    return hashPassword;
  };
  verifyPassword = async (password, actual_password) => {
    const is_correct_password = await (0, import_bcrypt.compare)(password, actual_password);
  ...
  generateAccessAndRefreshToken = function(user) {
    const refreshToken = (0, import_jsonwebtoken.sign)({ id: user.id }, env.JWT_REFRESH_TOKEN_SECRET, {
      expiresIn: "15d"
    });
...
async function checkAuth(req, res, next) {
  try {
    ...
    const user = import_jsonwebtoken2.default.verify(accessToken, env.JWT_ACCESS_TOKEN_SECRET);
    if (!user) {
      return next({
    ...
    return next({
      message: error instanceof import_jsonwebtoken2.default.JsonWebTokenError ? "Please authenticate to perform this action" : "Authentication Error",
      status: 401
    });
  }
}
...
router.get(
  "/github/callback",
  import_passport.default.authenticate("github", { failureRedirect: "/" }),
...
var ProfileController = class {
  ...
  async editProfile(req, res, next) {
    ...
    if (username !== user.username || avatar !== user.avatar || name !== user.name || email !== user.email || profession !== user.profession) {
      ...
      accessToken = (0, import_jsonwebtoken3.sign)(
        {
          id: user.id,
          username,
          name,
  ...
  async readmeHandler(req, res, next) {
    ...
    const sanitizedReadme = (0, import_sanitize_html.default)(readme, {
      allowedTags: [],
      allowedAttributes: {}
    });
...
var import_sanitize_html2 = __toESM(require("sanitize-html"));
...
var PostController = class {
  ...
  detectTags = (content) => {
    ...
    const doc = (0, import_compromise.default)((0, import_remove_markdown.default)(content));
  ...
  async createPost(req, res, next) {
    ...
    const sanitizedContent = (0, import_sanitize_html2.default)(content, {
      allowedTags: [],
      allowedAttributes: {}
    });
...
app.use((0, import_cors.default)({ origin: "http://localhost:5173", credentials: true }));
...
import_passport2.default.deserializeUser(
  (user, done) => {
    done(null, user);
  }
);
